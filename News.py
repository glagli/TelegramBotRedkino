...
def news(dp, keyboard, bot):
    ...
    async def cmd_news(message: types.Message):
        ...
        async def send_photo(message: types.Message):
            ...
            text = f"Публикация от пользователя @{message.from_user.username} \n\n{message.caption}"
            await bot.send_photo(message.from_user.id, photo_id, caption=text)
        ...
        async def send_voice(message: types.Message):
            ...
            message.from_user.id = -1001674751308
            text = f"Публикация от пользователя @{message.from_user.username} \n\n{message.caption}"
            await bot.send_voice(message.from_user.id, voice_id, caption=text)
            await message.answer("👀 Ваш пост успешно принят\n"
                                 "Хотите что-то ещё рассказать?")
            await message.answer("⌨️ Выбери что хотите сделать:", reply_markup=keyboard)
        ...
        async def send_file(message: types.Message):
            ...
            text = f"Публикация от пользователя @{message.from_user.username} \n\n{message.caption}"
            message.from_user.id = -1001674751308
            await bot.send_document(message.from_user.id, file_id, caption=text)
            await message.answer("👀 Ваш пост успешно принят\n"
                                 "Хотите что-то ещё рассказать?")
            await message.answer("⌨️ Выбери что хотите сделать:", reply_markup=keyboard)
        ...
        async def send_video(message: types.Message):
            ...
            text = f"Публикация от пользователя @{message.from_user.username} \n\n{message.caption}"
            message.from_user.id = -1001674751308
            await bot.send_video(message.from_user.id, video_id, caption=text)
            await message.answer("👀 Ваш пост успешно принят\n"
                                 "Хотите что-то ещё рассказать?")
            await message.answer("⌨️ Выбери что хотите сделать:", reply_markup=keyboard)
